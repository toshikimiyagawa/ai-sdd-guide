...
3b. Traceability — Before freezing, complete `specs/<feature>/traceability.json`
   (schema: `orchestration/schema/traceability.schema.json`). Run `sdd-validate.sh`
   (see #46) to confirm all Issue ACs are tracked and no orphaned out-of-scope entries
   exist. When presenting for human approval, include a diff summary of Issue ACs vs
...
- The sdd-reviewer must also verify (in addition to spec conformance):
  ...
  3. state.json feature/tier/phase matches the work under review.
